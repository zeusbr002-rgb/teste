...
export const Layout: React.FC<LayoutProps> = ({ children, user, onLogout }) => {
  ...
  // Simple Sidebar for desktop / Bottom nav for mobile
  const navItems = [
    { label: 'Ordens de Serviço', path: '/', icon: '📋' },
    { label: 'Cronograma', path: '/Cronograma', icon: '📅' },
    { label: 'Historico', path: '/Historico', icon: 'clock' },
    ...(user?.role === 'ADMIN' ? [
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-100">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-900 text-white shadow-xl z-20">
        ...
        <div className="p-4 border-t border-slate-800">
          ...
          <button onClick={onLogout} className="w-full text-xs text-slate-400 hover:text-white text-left pl-1">
            Sign Out
          </button>
        </div>
      </aside>
